...
export const WithIcons: Story = {
    render: () => ({
      ...
      <div class="relative h-screen bg-base-200">
        <div class="p-8">
          <h1 class="text-2xl font-bold mb-4">Dock with Icon Components</h1>
          <p class="mb-4">This dock uses the Icon component instead of SVG strings. Current active: <strong>{{ activeItem }}</strong></p>
        </div>
        
        <div class="fixed bottom-4 left-1/2 transform -translate-x-1/2 w-64">
          <Dock :items="items" size="lg" :active-item="activeItem" @item-click="handleItemClick" />
        </div>
      </div>
    `,
    }),
