<template>
  ...
  >
    ...
    <div v-if="loading" :class="loadingOverlayClasses">
      ...
      <span v-if="loadingText" class="mt-2 text-sm">{{ loadingText }}</span>
    </div>

    <!-- Image slot with lazy loading support -->
    <figure v-if="$slots.image" :class="imageContainerClasses">
      <slot name="image" />
      <div v-if="imageOverlay" :class="imageOverlayClasses">
