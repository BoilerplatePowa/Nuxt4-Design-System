<template>
  <div :class="filterClasses">
    ...
    <div v-if="showHeader" :class="headerClasses">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-2">
          ...
          <span v-if="activeFiltersCount > 0" class="badge badge-primary badge-sm">
            {{ activeFiltersCount }}
          </span>
        </div>
        ...
        <div class="flex items-center gap-2">
          ...
          <button v-if="collapsible" class="btn btn-ghost btn-xs" @click="toggleCollapsed">
    ...
    <div v-if="showActiveFilters && activeFilters.length > 0" :class="activeFiltersClasses">
      <div class="flex flex-wrap gap-2">
        ...
        >
          ...
          <button class="btn btn-ghost btn-xs w-4 h-4 p-0 min-h-0" @click="removeFilter(filter)">
            ×
          </button>
        </div>
      </div>
    ...
    <!-- Filter controls -->
    <div v-if="!collapsed" :class="controlsClasses">
      <div v-for="(group, groupIndex) in filterGroups" :key="groupIndex" :class="groupClasses">
        <!-- Group title -->
        <h4 v-if="group.title" class="font-medium mb-2">
          {{ group.title }}
        </h4>
        ...
        <div :class="getGroupControlsClasses(group)">
          ...
          >
            <!-- Text input -->
            <div v-if="filter.type === 'text'" class="form-control">
              <label v-if="filter.label" class="label label-text text-sm">{{ filter.label }}</label>
              <input
            ...
            <!-- Select dropdown -->
            <div v-else-if="filter.type === 'select'" class="form-control">
              <label v-if="filter.label" class="label label-text text-sm">{{ filter.label }}</label>
              <select
                v-model="filter.value"
            ...
            <div v-else-if="filter.type === 'multiselect'" class="form-control">
              <label v-if="filter.label" class="label label-text text-sm">{{ filter.label }}</label>
              <div class="space-y-1">
                <label
            ...
            <!-- Range input -->
            <div v-else-if="filter.type === 'range'" class="form-control">
              <label v-if="filter.label" class="label label-text text-sm">{{ filter.label }}</label>
              <div class="flex items-center gap-2">
                <input
            ...
            <!-- Date range -->
            <div v-else-if="filter.type === 'daterange'" class="form-control">
              <label v-if="filter.label" class="label label-text text-sm">{{ filter.label }}</label>
              <div class="flex items-center gap-2">
                <input
            ...
            <!-- Boolean toggle -->
            <div v-else-if="filter.type === 'boolean'" class="form-control">
